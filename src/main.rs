                        ...
                        let game_event_result =
                            (|| -> Result<macondo::GameEvent, Box<dyn std::error::Error>> {
                                ...
                                for (i, event) in game_history.events.iter().enumerate() {
                                    ...
                                    game_state.players[(event.nickname
                                        != game_history.players[0].nickname)
                                        as usize]
                                ...
                                game_state.bag.0.extend(
                                    (0u8..).zip(available_tally_buf.iter()).flat_map(
                                        |(tile, &count)| {
                                ...
                                println!(
                                    "{}",
                                    alphabet.fmt_rack(
                                        &game_state.players[game_state.turn as usize].rack
                                    )
                                );
                                ...
                                match &play {
                                    movegen::Play::Exchange { tiles } => {
                                        if tiles.len() == 0 {
                                            ...
                                            game_event
                                                .set_type(macondo::game_event::Type::Exchange);
                                            game_event.exchanged =
                                                format!("{}", alphabet.fmt_rack(tiles));
                                        }
                                    }
                                    movegen::Play::Place {
                                        down,
                                        ...
                                        game_event
                                            .set_type(macondo::game_event::Type::TilePlacementMove);
                                        let board_layout = game_config.board_layout();
                                        let dim = board_layout.dim();
                                        let strider;
                                        if *down {
                                            ...
                                            game_event.set_direction(
                                                macondo::game_event::Direction::Vertical,
                                            );
                                            game_event.position = format!(
                                                "{}{}",
                                                (*lane as u8 + 0x41) as char,
                                                idx + 1
                                            );
