...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while let Some(msg) = sub.next().await {
        ...
        let nwl18_kwg = std::sync::Arc::clone(&nwl18_kwg);
        let nwl20_kwg = std::sync::Arc::clone(&nwl20_kwg);
        let ecwl_kwg = std::sync::Arc::clone(&ecwl_kwg);
        ...
        tokio::spawn(async move {
            ...
            let mut buf = Vec::new();
            {
                ...
                let game_event_result =
                    (|| -> Result<macondo::GameEvent, Box<dyn std::error::Error>> {
                        ...
                        let (kwg, klv, game_config, mut game_state) =
                        ...
                        let mut move_filter = move_filter::GenMoves::Tilt(move_filter::Tilt::new(
                            &game_config,
                            &kwg,
                        ...
                        available_tally_buf
                            .extend((0..alphabet.len()).map(|tile| alphabet.freq(tile)));
                        ...
                        game_state
                            .bag
                            .0
                            .extend((0u8..).zip(available_tally_buf.iter()).flat_map(
                                |(tile, &count)| std::iter::repeat(tile).take(count as usize),
                            ));
                        ...
                        game_state.turn = match game_history.events.last() {
                            ...
                            Some(event) => {
                                (event.nickname == game_history.players[0].nickname) as u8
                            }
                        };
                        ...
                        if let move_filter::GenMoves::Tilt(ref mut tilt) = move_filter {
                        ...
                        let mut game_event = macondo::GameEvent {
                            rack: format!(
                                "{}",
                                alphabet
                                    .fmt_rack(&game_state.players[game_state.turn as usize].rack)
                            ),
                        ...
                        match &play {
                            ...
                            movegen::Play::Place {
                                ...
                                if *down {
                                    ...
                                    game_event
                                        .set_direction(macondo::game_event::Direction::Vertical);
                                    game_event.position =
                                    ...
                                    game_event
                                        .set_direction(macondo::game_event::Direction::Horizontal);
                                    game_event.position =
